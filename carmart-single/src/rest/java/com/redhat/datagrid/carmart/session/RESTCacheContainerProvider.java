...
public class RESTCacheContainerProvider extends CacheContainerProvider {
    volatile BasicCacheContainer container;
    ...
    public BasicCacheContainer getCacheContainer() {
        ...
        if (container == null) {
            synchronized (this) {
                if (container == null)
                    container = new RESTCacheContainer(edgProperty(EDG_HOST), edgProperty(HTTP_PORT),
                            edgProperty(REST_CONTEXT_PATH));
            }
        }
        return container;
    }
...
class RESTCache<K, V> implements Cache<K, V> {
    ...
    @Override
    public NotifyingFuture<Void> putAllAsync(Map<? extends K, ? extends V> map, long l, TimeUnit timeUnit, long l1,
            TimeUnit timeUnit1) {
        throw new UnsupportedOperationException("Unsupported");
    }
